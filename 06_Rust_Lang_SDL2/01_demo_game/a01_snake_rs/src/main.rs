// original code https://github.com/libsdl-org/SDL/tree/main/examples/demo/01-snake
use sdl2::{
...
fn main() {
    ...
    'running: loop {
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::KeyDown { scancode, .. } => match scancode {
                    Some(Scancode::Escape) => break 'running,
                    Some(Scancode::Right) => app_state.context.next_dir = SnakeDirection::Right,
                    Some(Scancode::Left) => app_state.context.next_dir = SnakeDirection::Left,
                    Some(Scancode::Up) => app_state.context.next_dir = SnakeDirection::Up,
                    Some(Scancode::Down) => app_state.context.next_dir = SnakeDirection::Down,
                    _ => {}
                },
                _ => {}
            }
        ...
        {
            // Add game logic update here (snake movement, collision detection, etc.)
        ...
        // Add code to render the snake and food here
