        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::MouseMotion {
                    ...
                    {
                        app_state.players[index].yaw -= (xrel as u32) * 0x00080000;
                        app_state.players[index].pitch = (-0x40000000).max(
                            0x40000000
                                .min(app_state.players[index].pitch - (yrel as i32) * 0x00080000),
                        );
                    } else if which != 0 {
