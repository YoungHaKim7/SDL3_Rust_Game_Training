...
SDL_GPUShader* LoadShader(SDL_GPUDevice* device,
    const std::string& shaderFilename, const Uint32 samplerCount,
    ...
    const Uint32 storageTextureCount)
{
    ...
    const SDL_GPUShaderFormat backendFormats { SDL_GetGPUShaderFormats(
        device) };
    SDL_GPUShaderFormat format {};
    std::string entrypoint { "main" };

    if (backendFormats & SDL_GPU_SHADERFORMAT_SPIRV) {
        fullPath = BasePath / "Content/Shaders/Compiled/SPIRV"
            / (shaderFilename + ".spv");
        format = SDL_GPU_SHADERFORMAT_SPIRV;
    } else if (backendFormats & SDL_GPU_SHADERFORMAT_MSL) {
        fullPath = BasePath / "Content/Shaders/Compiled/MSL"
            / (shaderFilename + ".msl");
        format = SDL_GPU_SHADERFORMAT_MSL;
        entrypoint = "main0";
    } else if (backendFormats & SDL_GPU_SHADERFORMAT_DXIL) {
        fullPath = BasePath / "Content/Shaders/Compiled/DXIL"
            / (shaderFilename + ".dxil");
...
SDL_Surface* LoadImage(
    const std::string_view imageFilename, const int desiredChannels)
{
...
int main()
{
    ...
    auto window { SDL_CreateWindow("Codotaku Game Engine", 800, 600,
    ...
    std::printf("Using GPU device driver: {}", SDL_GetGPUDeviceDriver(device));
    ...
    if (SDL_GPUTextureSupportsFormat(device,
            SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT, SDL_GPU_TEXTURETYPE_2D,
    ...
    auto vertexShader { LoadShader(
        device, "TexturedQuadWithMatrix.vert", 0, 1, 0, 0) };
    if (!vertexShader)
    ...
    auto pipeline { SDL_CreateGPUGraphicsPipeline(
    ...
    auto depthStencilTexture { SDL_CreateGPUTexture(
        device, &depthStencilTextureCreateInfo) };
    ...
    const auto* scene { importer.ReadFile(
        "Content/Models/viking_room.obj", aiProcess_Triangulate) };
    if (!scene)
    ...
    std::span vertexBufferData {
        reinterpret_cast<Vertex*>(transferBufferDataPtr), vertices.size()
    };
    std::ranges::copy(vertices, vertexBufferData.begin());

    std::span indexBufferData { reinterpret_cast<Uint32*>(transferBufferDataPtr
    ...
    auto isRunning { true };
    SDL_Event event;
    float windowAspectRatio { static_cast<float>(windowWidth)
    ...
    while (isRunning) {
        ...
        if (swapchainTexture) {
            std::array colorTargets { SDL_GPUColorTargetInfo {
                .texture = msaaTexture,
            ...
            auto renderPass { SDL_BeginGPURenderPass(commandBuffer,
                colorTargets.data(), colorTargets.size(),
                &depthStencilTarget) };

            SDL_BindGPUGraphicsPipeline(renderPass, pipeline);

            std::array<SDL_GPUBufferBinding, 1> bindings { { vertexBuffer,
                0 } };
            SDL_BindGPUVertexBuffers(
                renderPass, 0, bindings.data(), bindings.size());

            SDL_GPUBufferBinding indexBufferBinding { indexBuffer, 0 };
            SDL_BindGPUIndexBuffer(renderPass, &indexBufferBinding,
                SDL_GPU_INDEXELEMENTSIZE_32BIT);

            std::array<SDL_GPUTextureSamplerBinding, 1> textureSamplerBindings {
                { texture, sampler }
            };
            SDL_BindGPUFragmentSamplers(renderPass, 0,
                textureSamplerBindings.data(), textureSamplerBindings.size());

            auto projectionMatrix { glm::perspective(
                glm::radians(45.0f), windowAspectRatio, 0.1f, 100.0f) };
            auto viewMatrix { lookAt(glm::vec3 { 0.0f, 0.0f, 2.0f },
                glm::vec3 { 0.0f, 0.0f, 0.0f },
                glm::vec3 { 0.0f, 1.0f, 0.0f }) };
            auto modelMatrix { glm::mat4 { 1.0f } };
            modelMatrix = rotate(modelMatrix,
                glm::radians(static_cast<float>(ticks) * 0.1f),
                glm::vec3 { 0.0f, 1.0f, 1.0f });
            auto projectionViewMatrix { projectionMatrix * viewMatrix };
            auto modelViewProjectionMatrix { projectionViewMatrix
                * modelMatrix };
            SDL_PushGPUVertexUniformData(commandBuffer, 0,
                &modelViewProjectionMatrix, sizeof(modelViewProjectionMatrix));

            SDL_DrawGPUIndexedPrimitives(
                renderPass, indices.size(), 1, 0, 0, 0);
